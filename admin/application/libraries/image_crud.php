...
class image_CRUD {
	...
	protected $set_picker = false;
	protected $set_picker_selected = false;
	protected $set_image_selected = false;
	...
	{
		$this->set_picker = true;
	
		return $this;
	}	
	
	public function set_picker_selected()
	{
		$this->set_picker_selected = true;
	
		return $this;
	}	
	
	public function set_image_selected()
	{
		$this->set_image_selected = true;
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
    	...
    	return $this->ci->db->insert($this->table_name, $insert);
    }
    ...
    protected function _get_photos($relation_value = null, $relation_filter = null)
    {
    	...
    	if(!empty($relation_filter) && strpos($relation_filter, "-"))
    	{
    		echo $relation_filter;
    		$dados = explode("-",$relation_filter); 
    		$tipo = $dados[0];
    		$val = $dados[1];
    		$this->ci->db->join('imagem_relacao', ' imagem_relacao.idimagem = imagem.id');
    		$this->ci->db->where('imagem_relacao.idrelacao', $val);
    		$this->ci->db->where('imagem_relacao.tipo', $tipo);
    	}
...
$cc = 0;
	...
	protected function getState()
	{
		...
		if(isset($rsegments_array[4]) && strpos($rsegments_array[4], "-"))
		{
			...
			$relation_filter  = $rsegments_array[4];

			$state = array( 'name' => 'list', 'upload_url' => $upload_url, 'relation_value' => $this->ci->session->userdata('IDSite'),
		...
		elseif(isset($rsegments_array[3]) && is_numeric($rsegments_array[3]))
		{
			$upload_url = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/upload_file/'.$rsegments_array[3]);
			$ajax_list_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/'.$rsegments_array[3].'/ajax_list');
			$ordering_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/ordering');
		...
		elseif(isset($rsegments_array[3]) && $rsegments_array[3] == 'upload_file')
		{
			...
			//{
			$results['relation_value'] = $this->ci->session->userdata('IDSite');// $rsegments_array[4];
			//}
	...
	function render()
	{
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				case 'list':
					$rv = isset($state_info->relation_value) ? $state_info->relation_value : null;
					$rf = isset($state_info->relation_filter) ? $state_info->relation_filter : null;
					
					
					$photos = $this->_get_photos($rv, $rf); //isset($state_info->relation_value) ? $this->_get_photos($state_info->relation_value) : $this->_get_photos();
					$this->_library_view('list.php',array(
						...
						'set_picker' => $this->set_picker,
						'set_picker_selected' => $this->set_picker_selected,						
						'set_image_selected' => $this->set_image_selected,						
				...
				case 'upload_file':
					...
					@ob_end_clean();
					echo json_encode((object)array('success' => $result, 'data' => ($id)));					
